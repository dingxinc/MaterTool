...
namespace AsyncLog {
    ...
    public:
        LogLv _level;                     // 日志等级
        std::queue<std::any> _logdatas;   // 日志数据
    };
    ...
    public:
        ...
        template<typename Arg>  // 上面通过递归调用，最后会剩下一个参数的版本
        ...
        void AsyncWrite(LogLv level, Args&&... args) { // 万能引用的好处：支持右值传递、支持完美转发
    ...
    private:
        ...
        bool convert2Str(const std::any& data, std::string& str) {  // 将 any 类型转换为字符串类型
        ...
        std::string formatString(const std::string& format, Args... args) {
            ...
            (replacePlaceholder("{}", args), ...);  // 折叠表达式，递归替换 {}
            return result;
        }
